...
guessAllLetters:                                            @ ARM-to-C TRANSLATION by KEEGAN MCCALLUM
    @ args = 0, pretend = 0, frame = 8
    @ frame_needed = 1, uses_anonymous_args = 0
    @ link register save eliminated.
    str fp, [sp, #-4]!
    add fp, sp, #0
    sub sp, sp, #12                                         @ (reserve space for local vars and copies of params)
    mov r3, #97                                             @ (int i; for (i = 97;
    str r3, [fp, #-8]                                       @ )
    b   .L63
.L64:
    ldr r3, [fp, #-8]                                       @ (lettersAlreadyGuessed[i - 97] = i;
    sub r3, r3, #97
    ldr r2, [fp, #-8]
    uxtb    r1, r2
    ldr r2, .L65
...
hngDrawMan:                                                     @ ARM-to-C TRANSLATION by KEEGAN MCCALLUM
    @ args = 0, pretend = 0, frame = 16
    @ frame_needed = 1, uses_anonymous_args = 0
    stmfd   sp!, {fp, lr}
    add fp, sp, #4
    sub sp, sp, #16                                             @ (reserve space for local vars and copies of params)
    str r0, [fp, #-16]                                          @ (copy param lives to stack)
    ldr r3, [fp, #-16]                                          @ (if (lives > 6 ||
    cmp r3, #6
    bgt .L97                                                    @ )
    ldr r3, [fp, #-16]                                          @ ( lives < 0) {
    cmp r3, #0
    blt .L79                                                    @ )
.L69:
    mov r3, #32                                                 @ (char head = ' ';
    ...
    mov r3, #32                                                 @ (char rLeg = ' ';
    strb    r3, [fp, #-9]                                       @ )
    mov r3, #32                                                 @ (char torso = ' ';
    strb    r3, [fp, #-10]                                      @ )
    ldr r3, [fp, #-16]                                          @ (if (lives <= 5) {
    cmp r3, #5
    bgt .L71                                                    @ )
    mov r3, #48                                                 @ ( head = '0';
    strb    r3, [fp, #-5]                                       @ )
.L71:
    ldr r3, [fp, #-16]                                          @ (if (lives <= 4) {
    cmp r3, #4
    bgt .L72                                                    @ )
    mov r3, #124                                                @ (torso = '|';
    strb    r3, [fp, #-10]                                      @ )
.L72:
    ldr r3, [fp, #-16]                                          @ (if (lives <= 3) {
    cmp r3, #3
    bgt .L73                                                    @ )
    mov r3, #47                                                 @ (lArm = '/';
    strb    r3, [fp, #-6]                                       @ )
.L73:
    ldr r3, [fp, #-16]                                          @ (if (lives <= 2) {
    cmp r3, #2
    bgt .L74                                                    @ )
    mov r3, #92                                                 @ (rArm = '\\';
    strb    r3, [fp, #-7]                                       @ )
.L74:
    ldr r3, [fp, #-16]                                          @ (if (lives <= 1) {
    cmp r3, #1
    bgt .L75                                                    @ )
    mov r3, #47                                                 @ lLeg = '/';
    strb    r3, [fp, #-8]                                       @ )
.L75:
    ldr r3, [fp, #-16]                                          @ (if (lives == 0) {
    cmp r3, #0
    bne .L76                                                    @ )
    mov r3, #92                                                 @ (rLeg = '\\';
    strb    r3, [fp, #-9]                                       @ )
.L76:
    ldr r2, .L80                                                @ (printf("You have %d %s left!\n", lives, (lives == 1) ? "life" : "lives");
    ldr r3, [fp, #-16]
    cmp r3, #1
    bne .L77
    ldr r3, .L80+4
...
.L78:
    ...
    mov r2, r3
    bl  printf                                                  @ )
    ldr r0, .L80+12                                             @ ( printf("          ---|          \n");
    bl  puts                                                    @ )
    ldr r2, .L80+16                                             @ (printf("          |  %c         \n",
    ldrb    r3, [fp, #-5]   @ zero_extendqisi2                  @ head);
    mov r0, r2
    mov r1, r3
    bl  printf                                                  @ )
    ldr r0, .L80+20                                             @ (printf("          | %c%c%c\n",
    ldrb    r1, [fp, #-6]   @ zero_extendqisi2                  @ lArm,
    ldrb    r2, [fp, #-10]  @ zero_extendqisi2                  @ torso,
    ldrb    r3, [fp, #-7]   @ zero_extendqisi2                  @ rArm);
    bl  printf                                                  @ )
    ldr r1, .L80+24                                             @ (printf("          | %c %c\n",
    ldrb    r2, [fp, #-8]   @ zero_extendqisi2                  @ lLeg,
    ldrb    r3, [fp, #-9]   @ zero_extendqisi2                  @ rLeg);
    mov r0, r1
    mov r1, r2
    mov r2, r3
    bl  printf                                                  @ )
    ldr r0, .L80+28                                             @ (printf("          |-------\n");
    bl  puts                                                    @ )
    b   .L67                                                    @ (leave function)
.L79:
    mov r0, r0  @ nop                                           @ return;
.L67:
    sub sp, fp, #4
    ldmfd   sp!, {fp, pc}                                       @ restore old frame pointer
.L81:
